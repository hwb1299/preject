...
void CCJExcelToJsonDlg::OnBnClickedButtonStart()
{
	...
	if (IDOK == dlg.DoModal())
	{
		...
		{
			AfxMessageBox(L"转换完毕,json文件保存在exe文件目录下");
		}
		else
		{
			AfxMessageBox(L"转换失败，请确定excel文件是关闭状态");
		}
	}
}
					...
					for (int j = 0; j < this->m_nHeaderCount; j++)
					{
						...
						if (celltype == CELLTYPE_STRING)
						{
							jsonRow.jsonNode[j].strValue = sheet->readStr(i, j);
  							CString str = jsonRow.jsonNode[j].strValue;
							str.Replace(L"\"", L"\\\"");
							str.Replace(L"\\N", L"\\\\N");
							str.Replace(L"\\0", L"");
							str.Replace(L"\n", L"");
...
//保存json文件
void CCJExcelToJsonDlg::SaveJson(CString fileName)
{
	CString strOutFileName = m_strFileName;

	strOutFileName.Replace(strOutFileName, fileName + L".json");
	strOutFileName.Replace(strOutFileName, fileName + L".json");
